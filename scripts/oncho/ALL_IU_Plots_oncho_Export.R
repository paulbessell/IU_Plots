...
plotOutputPath <- paste0("Oncho/Plots/", Sys.Date(), "/Individual")
outpath <- if(!dir.exists(plotOutputPath)) dir.create(plotOutputPath, recursive = T)
plotOutputPath <- paste0("Oncho/Plots/", Sys.Date())

ius <- read_csv("Data/Oncho/IU_List.csv") # Necessary for curating IU names later. Just a global list

prevPath <- "Oncho/epioncho-MTP-v0-20241211-disaggregated-projections-prevalence-data/epioncho-MTP-v0-20241211-disaggregated/epioncho-MTP-v0-20241211-disaggregated"
prevalenceFiles <- list.files(prevPath) # Requires updating with folder names. Also below
iusList <- gsub(".csv|output_full_MTP_", "", prevalenceFiles)

geostat <- get(load("Data/Oncho/ALL_prevalence_map_multipletimespoints.rds"))
...
geostatcombGather <- geostatcomb %>%
  dplyr::select(-TaskID) %>%
  gather(key = "SimID", value = "Prev", -c("Year", "IU_CODE")) %>%
  mutate(SimID = gsub("V", "X", SimID)) %>%
  left_join(ius %>%
              dplyr::select(IU_ID, IU_CODE, IU_ID_Full)) %>%
  filter(!is.na(Prev))

onchohistory <- read_csv("Data/Oncho/final_df_oncho_Parent_Child.csv") %>% # IU history data
  mutate(IU_ID_Full = ifelse(IU_ID_MAPPING < 10000, paste0(ADMIN0ISO3, "0", IU_ID_MAPPING), paste0(ADMIN0ISO3, IU_ID_MAPPING)))

onchoSites <- read_csv("Data/Oncho/Oncho_Sites.csv") %>% # Site level data from ESPEN
  mutate(IU_ID_Full = ifelse(IU_2021 < 10000, paste0(ISO3, "0", IU_2021), paste0(ISO3, IU_2021))) %>%
  filter(Examined >= Positive) %>%
  mutate(monthN = match(SurveyMonth, month.name),
         xPos = SurveyYear + ((monthN - 6) / 13) +
           runif(nrow(.), -0.3, 0.3)) # %>%
  # left_join(ius, by = c("IU_2021" = "IU_ID"))
...
mda <- read_csv("Data/Oncho/Full_histories_df_popinfo_210324.csv")
...
pdf(file = paste0(plotOutputPath, "/Oncho_IU_Plots.pdf"),onefile = TRUE)
for(i in 1:length(iusList)){
# for(i in 1:100){
    cIU <- iusList[i]

    exprev <- read_csv(paste0(prevPath, "/output_full_MTP_", cIU, ".csv")) %>%
      filter(measure == "prevalence",
              year_id >= 2000) %>%
      dplyr::select(-c(age_start, age_end, measure)) %>%
      gather(key = "Sim_ID", value = "Prevalence", -c("year_id"))
